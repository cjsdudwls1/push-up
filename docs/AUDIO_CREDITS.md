...
- `11a67a4f301c` "조금만 더 올라가요!"(턱걸이가 인정 선에 못 미쳤을 때)는 위 파일들을 만든 뒤에 생긴
  대사라 아직 클립이 없고, 폰의 TTS가 읽습니다. 그래서 `--check`는 `clips: 101 of 103 present`를
  출력하고 이 줄과 `796f104f1460` 두 개를 `missing`으로 보여 줍니다. 녹음하면 `COACH`로 만듭니다.
